...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        x = x + self.attention(self.ln_1(x)) #test_encoder
...
def colorful_spectrum_mix(img1, img2, alpha, ratio=1.0):
    ...
    c, h, w = img1.shape[1:]
    h_crop = int(h * np.sqrt(ratio))   # crop size
...
class AttentionFusion(nn.Module):
    ...
    def q_k_v_product_attention(self, q_emb, k_emb, v_emb):
        ...
        weights = torch.einsum('bhid,bhjd->bhij', [q_emb, k_emb])
        weights = torch.div(weights, (self.embed_dim_qkv ** 0.5))
        weights = self.softmax(weights)
        new_v_emb = torch.einsum('bhid,bhjd->bhij', [weights, v_emb.permute(0, 1, 3, 2)])
        return new_v_emb

    def forward(self, text_features1, text_features2):
        text_features1 = text_features1.permute(1, 0, 2)  # LND -> NLD  
        text_features2 = text_features2.permute(1, 0, 2)

        batch_size = text_features1.size(0)
        q_emb = self.embedding_q(text_features1.unsqueeze(2))  # (128, 196, 1, 768)
        k_emb = self.embedding_k(text_features2.unsqueeze(2)) 
        v_emb = self.embedding_v(text_features2.unsqueeze(2))
        new_v_emb = self.q_k_v_product_attention(q_emb, k_emb, v_emb)
        new_text_features = self.embedding_common(new_v_emb)
        new_text_features = new_text_features.view(batch_size, -1,self.embed_dim) + text_features1


        new_text_features = new_text_features.permute(1, 0, 2)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cv_emb = None):
        x = self.conv1(x)  # shape = [*, width, grid, grid]   (128, 768, 14, 14)
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND  (128, 197, 768)->(197, 128, 768)
        
        x12 = self.transformer.resblocks[:12](x) 
...
class UBS(nn.Module):
    ...
    def forward(self, x): #（192， 256， 64， 32）
        ...
        B = x.size(0)
        mu = x.mean(dim=[2, 3], keepdim=True)#（192， 256， 1， 1）
        var = x.var(dim=[2, 3], keepdim=True)
        sig = (var + self.eps).sqrt()    #
        mu, sig = mu.detach(), sig.detach()
        x_normed = (x - mu) / sig     #对 (F-u(F))vir(F)
        ...
        mu_b = Distri_mu.sample([B, ])   #(196, 256)
        sig_b = Distri_std.sample([B, ]) 
        mu_b = mu_b.unsqueeze(2).unsqueeze(2)  #(196, 256, 1, 1)
    ...
    def forward_modify(self, x): #（192， 256， 64， 32）  ::NLD  #NLD
        if not self.training: 
            return x
        ...
        mu = x.mean(dim=[1], keepdim=True)#（128, 1, 768）
        var = x.var(dim=[1], keepdim=True)
        sig = (var + self.eps).sqrt()  

        x_normed = (x - mu) / sig     #(F-u(F))vir(F)


        mu_mu = mu.mean(dim = 0, keepdim=True).squeeze()
        sig_mu = sig.mean(dim = 0, keepdim=True).squeeze()

        mu_std = mu.std(dim=0,keepdim=True).squeeze()
        sig_std = sig.std(dim=0,keepdim=True).squeeze()

        Uniform_mu = Uniform(low=mu_mu-self.rho*mu_std, high=mu_mu + self.rho*mu_std)
        Uniform_sig = Uniform(low=sig_mu-self.rho*sig_std, high=sig_mu + self.rho*sig_std)

        mu_isg = Uniform_mu.sample([B, ]).reshape([B, 1,C])
        sig_isg = Uniform_sig.sample([B, ]).reshape([B, 1, C])

        x_final = x_normed * sig_isg + mu_isg
        x_final = x_final.permute(1, 0, 2)  # NLD -> LND  
        return x_final